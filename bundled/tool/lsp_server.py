...
from itchy.dummy_nodes import find_node, RECOVERY_STRATEGIES, AGGRESSIVE_STRATEGIES, make_args
...
func_signature_parser = Parser(skip_bad_tokens=False, skip_rules_on_fail=RECOVERY_STRATEGIES)
...
def get_function_info(parse_result: ParseResult | None, ast_builder: ASTBuilder):
    function_name = None
    active_parameter = None

    if parse_result is not None:
        assert isinstance(parse_result.tree, ParsedNode)
        try:
            ast_builder.reset()
            if (node := find_node(parse_result.tree, "functioncall")) is not None:
                tree = ast_builder.build_functioncall(node)
                function_name = tree.callee
                active_parameter = len(tree.args)
            elif (node := find_node(parse_result.tree, "eventstat")) is not None:
                tree = ast_builder.build_eventstat(node)
    ...
    if not (function_name and active_parameter):
        return None
...
def completions(params: types.CompletionParams) -> list[types.CompletionItem]:
    ...
    # current_function = get_function_info(completions_parser.deepest_partial or parsed, completion_ast)
    current_function = None
    ...
    try:
        ...
        if parsed is not None:
            ...
            try:
                ...
                function_name = tree.callee
                active_parameter = len(tree.args)
                current_function = CurrentFunction(function_name, active_parameter)
            except ValueError:
                pass



    # current_function = get_function_info(func_signature_parser.deepest_partial or parsed, func_signature_ast)
